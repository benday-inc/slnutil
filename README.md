...
| [enablecodeanalysis](#enablecodeanalysis) | Enable Roslyn code analysis across a solution. Default mode creates/merges Directory.Build.props at the solution root. Use --per-project to install analyzers directly into each csproj (required for packages.config projects). |
...
| [mcp-config](#mcp-config) | Print MCP client configuration for the slnutil server (Claude Code, Claude Desktop, VS Code, Visual Studio, Cursor), or install/uninstall it at user scope. Run with no arguments to print config for every supported client. |
| [mcp-server](#mcp-server) | Run slnutil as an MCP (Model Context Protocol) server over stdio. Exposes read-only solution, project, and file inspection tools to AI clients. This command runs until the client disconnects; it is normally launched by the MCP client, not by hand. See 'mcp-config' to register it with a client. |
...
## <a name="enablecodeanalysis"></a> enablecodeanalysis
...
| --- | --- | --- | --- |
| filename | Required | String | Path to file |
## <a name="updatebicepversions"></a> updatebicepversions
...
## <a name="snippetize"></a> snippetize
